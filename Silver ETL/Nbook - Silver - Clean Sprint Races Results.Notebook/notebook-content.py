...
# ###### _**Clean Sprint Races Results**_
# 
...
CalcPoints = (
    F.when(F.col("position") == 1, 8)
     .when(F.col("position") == 2, 7)
     .when(F.col("position") == 3, 6)
     .when(F.col("position") == 4, 5)
     .when(F.col("position") == 5, 4)
     .when(F.col("position") == 6, 3)
     .when(F.col("position") == 7, 2)
     .when(F.col("position") == 8, 1)
     .otherwise(0)
)
...
df_CleanStatusPos = (
    ...
    df_WithOut_Notes
        ...
        .withColumn(
            "Fix_Points",
            F.when( (F.col("points")  == 0 ) & (F.col("position") <= 8 ) & (F.col("season") >= 2022 ), "Y" )
            .otherwise("N")
        )
...
df_Races_Notes = (
    df_With_Notes
        ...
        .withColumn("Event", F.lit("Sprint Race"))
        .select( 
            F.col("Note"), 
